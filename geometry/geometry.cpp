...
vector <point> convex_hull(vector <point> pts) {
	...
	vector <point> ch(2 * n);
	for (int i = 0; i < n; i++) {
		while (t > 1 && sign(cross(ch[t-1]-ch[t-2], pts[i]-ch[t-2])) <= 0) {
			t--;
		}
		ch[t++] = pts[i];
	}
	int pt = t;
	for (int i = n - 2; i >= 0; i--) {
		while (t > pt && sign(cross(ch[t-1]-ch[t-2], pts[i]-ch[t-2])) <= 0) {
			t--;
		}
		ch[t++] = pts[i];
	}
...
int count_commont_tangents(point C1, LL r1,
													 point C2, LL r2) {
	LL d2 = dist2(C1, C2);
...
vector<point>half_planes_intersection(vector<line>all){
 ...
 all.push_back(line(point(-INF,INF),point(-INF,-INF)));
  int n = all.size();
	...
	while (head + 1 < tail && Q[head].side(
				 intersect(Q[tail - 1], Q[tail - 2])) != 1)
		tail--;
	while (head + 1 < tail && Q[tail - 1].side(
				 intersect(Q[head], Q[head + 1])) != 1)
		head++; /// not sure
