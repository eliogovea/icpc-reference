...
template <class T> int get_sub_tree_size(treap_node <T> * now) {
	return now == nullptr ? 0 : now->sub_tree_size;
}

template <class T> void update_sub_tree_size(treap_node <T> * now) {
	if (now != nullptr) {
		now->sub_tree_size = 1 + get_sub_tree_size(now->left) + get_sub_tree_size(now->right);
	}
}

template <class T> treap_node <T> * merge(treap_node <T> * left, treap_node <T> * right) {
	if (left == nullptr) {
		return right;
	}
	if (right == nullptr) {
...
template <class T> pair <treap_node <T> *, treap_node <T> *> split(treap_node <T> * tree, T key) {
	if (tree == nullptr) {
		return make_pair(nullptr, nullptr);
	}
	if (tree->key <= key) {
		pair <treap_node <T> *, treap_node <T> *> right_split = split(tree->right, key);
...
template <class T> struct treap {
	...
	treap_node <T> * insert(treap_node <T> * now, treap_node <T> * new_node) {
		...
		if (new_node->priority > now->priority) {
			pair <treap_node <T> *, treap_node <T> *> s = split(now, new_node->key);
	...
	void insert(T key) {
		root = insert(root, get_new_node(key));
	}
