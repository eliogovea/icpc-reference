...
point operator*(const point &P, const LL k) { return point(P.x * k, P.y * k); }
...
inline LL dot(const point &P, const point &Q) { return P.x * Q.x + P.y * Q.y; }
...
inline point rotate_ccw_90(const point &P) { return point(-P.y, P.x); }
...
/// test ok
inline bool segment_segment_intersect(const point &A, const point &B,
                                      const point &C, const point &D) {
    if (cross(B - A, D - C) == 0) {  // lines are parallel
        return (is_in(A, C, D) || is_in(B, C, D) || is_in(C, A, B) ||
                is_in(D, A, B));
    }
...
vector<point> convex_hull_graham_scan(vector<point> pts) {
    ...
    for (int i = 0; i < n; i++) {
        while (top >= 2 &&
               cross(ch[top - 1] - ch[top - 2], pts[i] - ch[top - 2]) <= 0) {
            top--;
        }
        ch[top++] = pts[i];
    }
...
void normalize_convex(vector<point> &polygon) {
    ...
    rotate(polygon.begin(), min_element(polygon.begin(), polygon.end()),
           polygon.end());
    int ptr = 1;
    for (int i = 1; i < polygon.size(); i++) {
        if (is_in(polygon[i], polygon[i - 1],
                  polygon[i + 1 == polygon.size() ? 0 : i + 1]))
            ;
...
bool convex_to_segment_intersection(const vector<point> &polygon,
    ...
    if (cross(B - A, polygon[0] - A) > 0) {
        ...
        while (lo <= hi) {
            int mid = (lo + hi) >> 1;
            if (cross(polygon[mid] - A, B - A) >=
                cross(polygon[mid - 1] - A, B - A)) {
                id = mid;
                lo = mid + 1;
            } else {
                hi = mid - 1;
    ...
    while (lo <= hi) {
        int mid = (lo + hi) >> 1;
        if (cross(B - A, polygon[mid]) >=
            cross(B - A, polygon[mid + 1 == polygon.size() ? 0 : mid + 1])) {
            id = mid;
            hi = mid - 1;
        } else {
            lo = mid + 1;
...
// }
// }
// double min_area = (double)(pts[up].y - pts[low].y) * (double)(pts[right].x -
// pts[left].x); for (int i = 0; i < n; i++) { /// i < n ??? point v[4]; v[low]
// = pts[ind[low] + 1 == n ? 0 : ind[low] + 1] - pts[ind[low]]; v[up] =
// pts[ind[up]] - pts[ind[up] + 1 == n ? 0 : ind[up] + 1]; v[right] =
// rotate_cw_90(pts[ind[right] + 1 == n ? 0 : ind[right] + 1] -
// pts[ind[right]]); v[left] = rotate_ccw_90(pts[ind[left] +1 == n ? 0 :
// ind[left] + 1] - pts[ind[left]]); int min_angle_id = 0; for (int j = 1; j <
// 4; j++) { if (cross(v[j], v[min_angle_id]) > 0) { min_angle_id = j;
// }
// }
// point V = pts[ind[min_angle_id] + 1 == n ? 0 : ind[min_angle_id] + 1] -
// pts[ind[min_angle_id]]; double w = (double)abs(cross(v, pts[(min_angle_id +
// 2) % 4] - pts[min_angle_id])) / (double)norm(V); double h =
// (double)abs(dot(v, pts[(min_angle_id + 1) % 4] - pts[(min_angle_id + 3) %
// 4])) / (double)norm(V); min_area = min(min_area, w * h);
// }
// return min_area;
// }
...
/// ex c = minkowsky_sum(a, b) => every point in c is the sum of two points from
/// a and b respectively and every pair of points from a and b respectively is a
/// point of c (not only the boundary)
    ...
    while (q--) {
        ...
        if (s != 0) {
            ...
            if (dot(B - A, C - A) < 0) {
                ...
                cout << (dist2(A, B) < dist2(A, C) ? "ONLINE_FRONT"
                                                   : "ON_SEGMENT")
                     << "\n";
            }
        }
...
void test_signed_area_2() {
    ...
    while (cin >> n) {
        ...
        for (int i = 1; i < pts.size(); i++) { pts[i] = pts[i] - pts[0]; }
        pts[0] = point(0, 0);
...
void test_maxcolinear() {
    ...
    while (cin >> n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (auto it : m) { answer = max(answer, it.second + 1); }
        }
        cout << answer << "\n";
    }
}
