...
inline LL dot_LL(point_LL P, point_LL Q) { return P.x * Q.x + P.y * Q.y; }

inline LL cross_LL(point_LL P, point_LL Q) { return P.x * Q.y - P.y * Q.x; }

inline LD length(point_LL P) { return sqrtl((LD)dot_LL(P, P)); }
...
point_LD operator*(point_LD P, double k) { return point_LD(P.x * k, P.y * k); }

inline point_LD get(point_LL P) { return point_LD(P.x, P.y); }

inline LD dot_LD(point_LD P, point_LD Q) { return P.x * Q.x + P.y * Q.y; }

inline LD cross_LD(point_LD P, point_LD Q) { return P.x * Q.y - P.y * Q.x; }
...
LD segment_segment_distance(point_LL A, point_LL B, point_LL C, point_LL D) {
    ...
    return min(min(point_segment_distance_LL(A, C, D),
               ...
               min(point_segment_distance_LL(C, A, B),
                   point_segment_distance_LL(D, A, B)));
}
