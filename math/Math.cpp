...
LL phi(LL n) { // euler phi
	auto f = factorize(n);
	LL r = n; for (auto p : f) r -= r / p.first;
	return r;
}
...
LL primitiveRoot(LL n) {
	...
	auto f = factorize(n);
	if (f[0].first == 2 && (f[0].second != 1 || f.size() != 2))
		return -1;
	...
	for (int g = 2; g < n; g++) {
		...
		bool ok = true; 
		for (auto & p : f) if (power(g, phin / p.first, n) == 1) {
			ok = false; break;
		}
		if (ok) return g;
	}
...
LL discreteSqrt(LL a, LL p) { // returns -1 if no solution
	...
	LL d = p - 1; int s = 0;
	while (!(d & 1)) s++, d >>= 1LL;
	LL q = 2; while (legendre(q, p) != p - 1) q++;
